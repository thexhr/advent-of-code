// XXX Check the XXX marker in the source, I hard coded some values
...
main(int argc, char **argv)
{
	...
	while ((read = getline(&line, &len, fd)) != -1) {
		...
		// Move in blocks by putting all elements in a temp array
		// and push the elements in reverse order
		int i = 0, j=0;
		for (i=0; i < amount; i++) {
			temp[i] = pop(from-1);
		}
